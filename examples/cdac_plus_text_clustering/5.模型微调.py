...
serialization_dir = 'fine_tuning'
kmeans_cluster_centers_pkl_filename: str = None
pretrain_model_filename = 'pretrain/pretrain_epoch_44.bin'
...
with open(pretrain_model_filename, 'rb') as f:
...
if kmeans_cluster_centers_pkl_filename is None:
    vector_represents = []
    ...
    with open(os.path.join(serialization_dir, 'kmeans_cluster_centers_.pkl'), 'wb') as f:
        pickle.dump(cluster_centers_, f)
else:
    with open(kmeans_cluster_centers_pkl_filename, 'rb') as f:
...
with open(os.path.join(serialization_dir, 'initial_metrics.json'), 'w', encoding='utf-8') as f:
...
t_total = int(len(train_all_dataset) / batch_size) * num_epochs
optimizer = BertAdam(
    optimizer_grouped_parameters,
    lr=learning_rate,
...
for idx_epoch in range(num_epochs):
    ...
    with open(os.path.join(serialization_dir, 'metrics_epoch_{}.json'.format(idx_epoch)), 'w', encoding='utf-8') as f:
        json.dump(metrics, f, indent=4, ensure_ascii=False)
    ...
    model_filename = os.path.join(serialization_dir, 'fine_tuning_epoch_{}.bin'.format(idx_epoch))
